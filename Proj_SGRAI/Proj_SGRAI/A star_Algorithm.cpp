...
Astar_Algorithm::Astar_Algorithm(int** board) { Astar_Algorithm::getBoardMap(board); }

void Astar_Algorithm::getBoardMap(int** board) { 
	...
	const int & xFinish, const int & yFinish)
{
	...
	while (!pq[pqi].empty())
	{
		...
		for (i = 0; i<dir; i++)
		{
			...
			if (!(xdx<0 || xdx>Board::BOARD_X - 1 || ydy<0 || ydy>Board::BOARD_Y - 1 || map[xdx][ydy] == 1 || map[xdx][ydy] == 5
